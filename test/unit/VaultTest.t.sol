...
import "test/mocks/VaultMock.sol";
...
abstract contract StateZero is Test {
    ...
    function setUp() public virtual {
        ...
        stdstore
        .target(address(usdc))
        .sig(usdc.balanceOf.selector).with_key(address(vault)).checked_write( //select balanceOf mapping
                100000 * 1e18
...
contract StateZeroTest is StateZero {
    ...
    function testUserDepositsWETH() public {
        ...
        stdstore
        .target(address(weth))
        .sig(weth.balanceOf.selector).with_key(address(user)).checked_write( //select balanceOf mapping
                1 ether
            ); //set mapping key balanceOf(address(user)) //data to be written to the storage slot -> balanceOf(address(user)) = 1 ether | assertTrue(weth.balanceOf(user) == 1 ether);
...
abstract contract StateDeposited is StateZero {
    function setUp() public virtual override {
        ...
        .sig(weth.balanceOf.selector).with_key(address(user)).checked_write( //select balanceOf mapping
                1 ether
            ); //set mapping key balanceOf(address(user)) //data to be written to the storage slot -> balanceOf(address(user)) = 1 ether
...
contract StateRateChangeTest is StateRateChange {
    function testCannotBorrowFurtherOnRateChange() public {
        console2.log(
            "Rate appreciates: User is unable to borrow another half of original debt amount due to rate change"
        );
        console2.log(
            "Rate appreciates: Twice the amount of collateral is needed to support initial debt position. There is no free collateral."
        );
...
contract StateRateChange2Test is StateRateChange2 {
    function testBorrowLessAtNewRate() public {
        console2.log(
            "Rate appreciates: User can borrow a lesser amount of DAI than before"
        );
...
contract StateLiquidatedTest is StateLiquidated {
    function testLiquidationOnPriceAppreciation() public {
        console2.log(
            "Price appreciated significantly: user should be liquidated"
        );
        uint userDebt = vault.debts(user);
        uint userDeposit = vault.deposits(user);
